...
static int configureUdaServerSSLContext(const Config& config);
...
SSL_CTX* createUdaServerSSLContext()
{
    ...
    SSL_CTX_set_options(g_ctx, SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_COMPRESSION);
    if (SSL_CTX_set_cipher_list(g_ctx, "HIGH:!aNULL:!kRSA:!PSK:!SRP:!MD5:!RC4") == 0) {
        throw std::runtime_error{ "failed to select cypher" };
    }
...
int configureUdaServerSSLContext(const Config& config)
{
    auto cert = config.cert;
    auto key = config.key;
    auto ca = config.cert;
    auto crlist = config.crlist;

    if (cert.empty() || key.empty() || ca.empty() || crlist.empty()) {
        if (cert.empty()) {
    ...
    X509_CRL* crl = loadUdaServerSSLCrl(crlist.c_str());
    if (!crl) {
...
int startUdaServerSSL(const Config& config)
{
    int rc;
    ...
    if (configureUdaServerSSLContext(config) != 0) {
    ...
    if (peer != nullptr) {
        ...
        UDA_LOG(UDA_LOG_DEBUG, "X509 not before: %ld\n", *(long *)X509_get_notBefore(peer)->data, NULL);
        UDA_LOG(UDA_LOG_DEBUG, "X509 not after: %ld\n", *(long *)X509_get_notAfter(peer)->data, NULL);
        X509_free(peer);
    } else {
        X509_free(peer);
