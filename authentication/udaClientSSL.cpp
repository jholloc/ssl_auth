...
int configureUdaClientSSLContext(const HostData& host)
{
    ...
    std::string cert = host.certificate;
    std::string key = host.key;
    std::string ca = host.ca_certificate;
    ...
    UDA_LOG(UDA_LOG_DEBUG, "Current Time               : %s\n", c_time_string);
    UDA_LOG(UDA_LOG_DEBUG, "Client certificate date validity checked\n");
...
int startUdaClientSSL()
{
    ...
    SSL_set_tlsext_host_name(g_ssl, g_host.host_name.c_str());
